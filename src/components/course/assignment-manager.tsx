...
function isLate(submission: Pick<Submission, "submittedAt">, dueDate: string | null) {
  return dueDate !== null && new Date(submission.submittedAt).getTime() > new Date(dueDate).getTime();
}

function lateBy(submittedAt: string, dueDate: string) {
  const minutes = Math.round((new Date(submittedAt).getTime() - new Date(dueDate).getTime()) / 60_000);
...
function GradeForm({ submission, onGraded }: { submission: Submission; onGraded: (submission: Submission) => void }) {
  ...
  return (
    <form onSubmit={save} className="mt-3 grid gap-2 rounded-xl bg-surface/60 p-3 sm:grid-cols-[100px_1fr_auto]">
      ...
      <label><span className="mb-1 block text-[11px] font-semibold text-muted">Feedback</span><input maxLength={10000} value={feedback} onChange={(event) => setFeedback(event.target.value)} placeholder="Actionable feedback" className="w-full rounded-lg border border-black/10 bg-white px-2.5 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-purple/20" /></label>
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={createAssignment} className="grid gap-3 rounded-2xl border border-black/5 bg-white p-5 lg:grid-cols-2">
        ...
        <label className="block"><span className="mb-1 block text-xs font-semibold text-muted">Title</span><input required maxLength={200} value={title} onChange={(event) => setTitle(event.target.value)} className="w-full rounded-xl border border-black/10 px-3 py-2.5 outline-none focus:ring-2 focus:ring-brand-purple/20" /></label>
